/**
 * Shared WhatsApp completion processing.
 *
 * Extracted so the two entry points cannot diverge:
 *
 *   - /api/process-whatsapp-complete  — EXTERNAL webhook. Requires a valid
 *     `x-webhook-secret` (RAJ-739, mandatory + fail-closed). Untouched by the
 *     route split.
 *   - /api/process-whatsapp-inapp     — IN-APP browser upload. Requires a valid
 *     `x-project-token` (RAJ-747: the token is only minted after a server-verified
 *     passphrase proof). No webhook secret — a browser must never hold one.
 *
 * Both routes perform their own authentication FIRST and then call
 * `processWhatsappCompletion()`. This module performs NO transport-level
 * authentication of its own; it only accepts an optional per-project
 * authorization callback invoked once the projectId is known.
 *
 * Zero-knowledge is preserved: nothing here accepts or inspects a raw
 * passphrase or passphrase hash.
 */
...
export interface ProcessWhatsappOptions {
  /**
   * Optional per-project authorization hook, invoked once the projectId has
   * been parsed out of the request body. Return a NextResponse to abort.
   */
  authorizeProject?: (projectId: string) => Promise<NextResponse | null>
}

/**
 * Parse a WhatsApp completion payload (JSON or legacy FormData), persist the
 * messages, and update the project's analysis.
 *
 * Callers MUST have already authenticated the request.
 */
export async function processWhatsappCompletion(
  request: NextRequest,
  ...
  try {
    ...
    if (contentType.includes('application/json')) {
      ...
      const totalMessagesCount = body.messages
        ? body.messages.length
        : chatData?.totalMessages || messages.length || 0

      dbAnalysis = {
        participants: clientAnalysis.participants || [],
        dateRange: clientAnalysis.dateRange || { start: '', end: '' },
        sentiment: clientAnalysis.sentimentAnalysis ||
          clientAnalysis.sentiment || { positive: 0, negative: 0, neutral: 100 },
    ...
    if (options.authorizeProject) {
      const authError = await options.authorizeProject(projectId)
    ...
    if (dbAnalysis.keywords.length === 0 && dbAnalysis.insights.length === 0) {
      ...
      if (
        existingProj &&
        existingProj.analysis &&
        (existingProj.analysis as any).keywords?.length > 0
      ) {
...
export function generateComprehensiveAnalysis(
  ...
  const words = allText
    ...
    .filter(
      (word) =>
        word.length > 3 &&
        !['this', 'that', 'with', 'have', 'will', 'from', 'they', 'been', 'were', 'said'].includes(
          word
        )
    )
...
export function generateInsights(analysis: ProcessingResult): string[] {
  ...
  if (mostActive) {
    ...
    insights.push(
      `${mostActive.name} is the most active participant with ${percentage}% of all messages`
    )
  }
