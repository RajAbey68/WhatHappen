# WhatHappen MCP Server Architecture & Implementation Plan (v9.0 - Final Hardened & Reconciled)
...
- `WHATHAPPEN_INGEST_ALLOWLIST`: **Mandatory**. JSON-encoded array of absolute directory paths (e.g. `["/Users/rajabey/exports"]`). If unset, empty, or invalid, the ingest tool returns an explicit `INGEST_DISABLED` error.
...
- **Key Derivation (Node.js Native Crypto):**
  ...
  - Buffer Zeroing: `derivedBuffer.fill(0)` executed immediately upon import. *(Defense-in-depth: memory protection relies on OS encrypted swap/FileVault on macOS).*
...
- **Atomic Open-Then-Verify Security Validation (TOCTOU Defense):**
  ...
  - Fails closed with `INGEST_DISABLED` if `WHATHAPPEN_INGEST_ALLOWLIST` is unset.
  - Extension check: Must strictly end with `.zip`.
  - **Open Handle First:** `const fd = fs.openSync(filePath, 'r')`.
  - **Inode & Path Validation:** Resolves `fs.fstatSync(fd).ino` and confirms `realPath = fs.realpathSync(filePath)` matches the same device/inode (`fs.statSync(realPath).ino === fs.fstatSync(fd).ino`).
  - **Boundary Check:** Verifies `realPath` is strictly within one of the parsed allowlist directories.
