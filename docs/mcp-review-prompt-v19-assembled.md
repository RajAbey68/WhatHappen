...
<plan>
# WhatHappen MCP Server Architecture & Implementation Plan (v19.0 - Convergence Master)
...
- **Exact Interoperable Server Protocol Alignment:**
  1. **HMAC Authentication Handshake:**
     ...
     - Memory Hygiene: `keyHex` and passphrase buffers are zeroed (`buf.fill(0)`) in `finally` blocks.
  2. **AES-GCM Decryption Key Derivation:**
     ...
     - Per-Project Key Cache: Key derivation is cached per unique `salt` in RAM to optimize message batch decryption while wiping raw derived buffers immediately upon WebCrypto import.
- **In-Memory Session Store & Auto-Eviction:**
  - Keyed by normalized lowercase UUID: `projectId.toLowerCase()`.
  ...
  - Schema: `{ token: string, saltMap: Map<string, CryptoKey>, expiresAt: number, passphraseHash: string }`.
...
- **Implementation:** Normalizes UUID to lowercase, returns `{ projectId, unlocked: boolean, expiresAt?: number, ttlRemainingSeconds?: number }` using local in-memory session cache.
...
- **Implementation:**
  ...
  6. **Mandatory Post-Token Verification:** Makes an authenticated verification call using the token (`GET /api/projects`) to confirm the server accepts the token before claiming success.
...
- **Implementation:**
  1. Normalizes UUID to lowercase, validates UTC date range consistency, and verifies session presence.
  ...
  5. Decrypts messages incrementally in RAM: for each message, derives/fetches key for `message.salt` and decrypts `message.ciphertext` with `message.iv` via AES-GCM (throws `DECRYPTION_FAILED` on tag mismatch).
  6. Truncates strictly at whole message boundaries before cumulative plaintext exceeds 100,000 UTF-8 bytes measured via `Buffer.byteLength(str, 'utf8')`.
...
- **Implementation:** Normalizes UUID to lowercase, validates ISO-8601 UTC timestamp and windowSize, fetches surrounding ciphertext window via API, decrypts each record in RAM with salt/iv tag verification, returns structured envelope.
...
- **Implementation (Client-Side Zero-Knowledge):**
  1. Normalizes UUID to lowercase, passes search boundaries (`startDate`, `endDate`, `limit`) to `POST /api/ai-chat/query` using cached token and idempotency key.
