...
<plan>
# WhatHappen MCP Server Architecture & Implementation Plan (v17.0 - Final Certified Master)
...
- **WhatHappen Server Target:** Strictly **Loopback Only** (`http://127.0.0.1:3000`).
  *(Hostnames like `localhost` are rejected to prevent DNS rebinding; remote access to Hermes-Dev requires an SSH local port forward: `ssh -N -L 3000:127.0.0.1:3000 -o StrictHostKeyChecking=yes user@167.233.236.178`).*
...
- `WHATHAPPEN_API_URL`: Base URL. Must parse to exact loopback IP `http://127.0.0.1:3000`. Any other target immediately halts process with `INVALID_API_URL`.
- `WHATHAPPEN_PASSPHRASE`: Project decryption passphrase, injected into local process RAM only.
  - Pre-flight Validation: Min 16 chars, max 256 chars. If empty or invalid, the process immediately halts on startup with exit code 1.
...
- **Verified Endpoint Protocol:**
  ...
  - Memory Hygiene: Sensitive buffers (`passphrase`, intermediate `keyHex`) are securely overwritten using `crypto.randomFillSync(buf)` in `finally` blocks on both success and error paths.
- **Key Derivation (PBKDF2):**
  - Algorithm: PBKDF2 with SHA-256 (100,000 iterations).
  - Derived Key: AES-GCM 256-bit `CryptoKey` (marked non-extractable).
  - Buffer Zeroing: `crypto.randomFillSync(derivedBuffer)` executed immediately upon import.
- **In-Memory Session Store & Auto-Eviction:**
  - Normalized Key: Keyed by strictly lowercase normalized `projectId.toLowerCase()`.
  - Capacity Bound: Hard limit of max 10 concurrent active sessions (LRU eviction).
  ...
  - Background Refresh: Queued at 90% TTL. On 401/403 or non-2xx failures, the session is immediately purged from memory.
...
- **Implementation:**
  ...
  4. Computes `proof = HMAC-SHA256(sha256Hex(passphrase), challenge.nonce)`. Overwrites buffers in `finally` block with `crypto.randomFillSync`.
  5. Calls `POST /api/project-token` with `{ projectId, challenge: nonce, proof }`.
  6. Derives non-extractable AES-GCM `CryptoKey` in RAM, overwrites raw buffer.
...
- **Parameters:**
  ...
  - `startDate` (string, optional): Strict ISO-8601 UTC date string (e.g. `2026-01-01T00:00:00Z`).
  - `endDate` (string, optional): Strict ISO-8601 UTC date string (must be >= `startDate` in UTC).
...
- **Implementation:**
  1. Validates UUID (normalized to lowercase), UTC date range consistency, and session presence.
  ...
  4. Decrypts messages incrementally in RAM using cached derived key with authenticated AES-GCM tag verification (throws `DECRYPTION_FAILED` on tag mismatch).
...
- **Parameters:**
  - `projectId` (string, required): Project UUID.
  - `anchorTimestamp` (string, required): Strict ISO-8601 UTC timestamp (e.g., `2026-09-01T12:00:00.000Z`).
...
- **Parameters:**
  ...
  - `startDate` (string, optional): Strict ISO-8601 UTC date string.
  - `endDate` (string, optional): Strict ISO-8601 UTC date string.
- **Fail-Fast Invariant:** Throws `PROJECT_LOCKED` or `SESSION_EXPIRED` if not unlocked.
