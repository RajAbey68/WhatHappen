...
<plan>
# WhatHappen MCP Server Architecture & Implementation Plan (v10.0 - Golden Master)
...
- **WhatHappen Server Target:** Strictly **Loopback Only** (`http://127.0.0.1:3000`, `http://localhost:3000`, or local `https://localhost:3000`).
  *(Remote direct IP access is eliminated. Connecting to Hermes-Dev requires an SSH local port forward: `ssh -L 3000:localhost:3000 root@167.233.236.178`).*
...
- `WHATHAPPEN_API_URL`: Base URL (default: `http://127.0.0.1:3000`).
  - Strict Protocol & Hostname: Parsed via Node.js `new URL(apiUrl)`. Protocol must be `http:` or `https:`. Hostname must strictly equal `localhost` or `127.0.0.1`.
- `WHATHAPPEN_PASSPHRASE`: Project decryption passphrase, injected into local process RAM only.
  - Pre-flight Validation: Minimum 16 characters with mixed characters.
- `WHATHAPPEN_INGEST_ALLOWLIST`: **Mandatory**. JSON-encoded array of absolute directory paths (e.g. `["/Users/rajabey/exports"]`). Parsed eagerly at server startup; if missing or invalid, ingest tool fails closed with `INGEST_DISABLED`.
...
- **Key Derivation & Trial Decryption Verification:**
  ...
  - **Trial Decryption Gate:** Upon deriving the AES key, the server performs a test decryption of a sample ciphertext or project verification metadata. If decryption fails, the key is immediately discarded, no session is cached, and an explicit `PASSPHRASE_INVALID` error is thrown.
...
- **Implementation:**
  ...
  8. Stores token + key in session cache with matching 2-hour TTL.
...
- **Security Validation & TOCTOU Defense:**
  - Project must be unlocked (`x-project-token` required in upload header; server validates token binds to `projectId`).
  - Pre-flight Allowlist Validation: Path must be strictly inside an allowlisted directory before any I/O.
  - Canonical resolution: `realPath = fs.realpathSync(filePath)`.
  - Allowlist Containment Check: Confirms `realPath` is strictly within an allowlist directory using `path.relative`.
  - Extension check: Must strictly end with `.zip`.
  - Atomic File Open: Uses `fs.openSync(realPath, 'r')`.
