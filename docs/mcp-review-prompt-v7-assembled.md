...
<plan>
# WhatHappen MCP Server Architecture & Implementation Plan (v7.0 - Final Certified)
...
- `WHATHAPPEN_API_URL`: Base URL (default: `http://127.0.0.1:3000`).
  - Strict Hostname Validation: Parsed via Node.js `new URL(apiUrl)`. Hostname must strictly equal `localhost` or `127.0.0.1`.
- `WHATHAPPEN_PASSPHRASE`: Project decryption passphrase, injected into local process RAM only.
  - Pre-flight Validation: Minimum 12 characters.
...
- **HMAC Proof Generation:**
  ...
  - Memory Hygiene: Both passphrase buffer and intermediate HMAC key buffers are zeroed (`buf.fill(0)`) immediately after HMAC computation.
- **Key Derivation (WebCrypto Native):**
  - Algorithm: PBKDF2 with SHA-256 (100,000 iterations).
  - WebCrypto Import: The raw derived PBKDF2 bytes are immediately imported via `crypto.subtle.importKey('raw', derivedBuffer, 'AES-GCM', false, ['encrypt', 'decrypt'])` with `extractable: false`.
  - Raw Buffer Zeroing: `derivedBuffer.fill(0)` is executed immediately upon import so no extractable raw key material persists.
...
- **In-Memory Session Store & Auto-Refresh:**
  ...
  - Concurrency Lock: Single in-flight unlock promise per `projectId`.
...
- **Atomic Authorization & Strict Path Validation:**
  - Project must be unlocked (`x-project-token` required in upload header).
  - Fails closed if `WHATHAPPEN_INGEST_ALLOWLIST` is unset.
  - Resolves canonical path: `const realPath = fs.realpathSync(filePath)`.
  - Strict Boundary Check: Checks that relative path from allowlist directory does not start with `'..'` and is not absolute (`!path.relative(allowedDir, realPath).startsWith('..') && !path.isAbsolute(path.relative(allowedDir, realPath))`).
  - Open Handle with `O_NOFOLLOW`: `const fd = fs.openSync(realPath, fs.constants.O_RDONLY | fs.constants.O_NOFOLLOW)`.
