...
<plan>
# WhatHappen MCP Server Architecture & Implementation Plan (v18.0 - Absolute Convergence)
...
- **WhatHappen Server Target:** Strictly **Loopback Only** (`http://127.0.0.1:3000`).
  *(Connecting to Hermes-Dev requires an SSH local port forward: `ssh -N -L 3000:127.0.0.1:3000 -o StrictHostKeyChecking=yes user@167.233.236.178`).*
...
- `WHATHAPPEN_API_URL`: Base URL. Must parse to exact loopback IP `http://127.0.0.1:3000` (parsed via `new URL()`; protocol must strictly be `http:`, hostname `127.0.0.1`, port `3000`, pathname `/`). Any deviation immediately halts process.
...
- **Verified Server Protocol Alignment:**
  - Ground-truth: The WhatHappen server (`lib/session-store.ts` and `app/api/project-token/route.ts`) explicitly specifies HMAC key as `sha256Hex(passphrase)`. To maintain 100% interoperability with the live server:
    - Key: `crypto.createHash('sha256').update(passphrase, 'utf8').digest('hex')`.
    - Signature: `crypto.createHmac('sha256', keyHex).update(challenge.nonce, 'utf8').digest('hex')`.
  - Memory Hygiene: `buf.fill(0)` is executed on all passphrase and intermediate buffers in `finally` blocks. *(Documented limitation: Node.js V8 memory garbage collection makes zeroing best-effort defense-in-depth; single-user workstation security model applies).*
...
- **In-Memory Session Store & Auto-Eviction:**
  - Keyed by lowercase UUID `projectId.toLowerCase()`.
  - Single-user local process binding (`process.uid` verified).
...
- **Implementation:** Returns `{ projectId, unlocked: boolean, expiresAt?: number, ttlRemainingSeconds?: number }` using local in-memory session cache.
...
- **Parameters:**
  - `projectId` (string, required): RFC 4122 compliant UUID string.
- **Implementation:**
  1. Validates UUID format and normalizes strictly to lowercase.
  ...
  7. **Mandatory Post-Token Verification:** Makes an authenticated verification call using the token (`GET /api/projects`) to confirm the server accepts the token before claiming success.
  8. Stores token + key in session cache under lowercase UUID with clamped `expiresAt` TTL.
  9. Returns `{ projectId, status: 'unlocked', expiresAt, ttlRemainingSeconds }`.
...
- **Implementation:** Normalizes to lowercase, deletes session from Map, and returns `{ projectId, status: 'locked' }`.
...
- **Parameters:**
  ...
  - `startDate` (string, optional): ISO-8601 UTC date string formatted as `YYYY-MM-DDTHH:mm:ss.sssZ` (or normalized to UTC).
  - `endDate` (string, optional): ISO-8601 UTC date string (must be >= `startDate` in UTC).
...
- **Implementation:**
  1. Validates UUID, UTC date range consistency, and session presence.
  2. Sends `POST /api/ai-chat/query` with server-side limit parameter, idempotency key (UUID v4), and token.
  3. Uses `AbortController` in a `try...finally` block; aborts and cleans up stream if incoming bytes exceed 50MB.
  4. Decrypts messages incrementally in RAM using cached derived key with authenticated AES-GCM tag verification (throws `DECRYPTION_FAILED` on tag mismatch or corruption).
...
- **Parameters:**
  - `projectId` (string, required): Project UUID.
  - `anchorTimestamp` (string, required): ISO-8601 UTC timestamp formatted as `YYYY-MM-DDTHH:mm:ss.sssZ`.
...
- **Implementation:** Validates ISO-8601 UTC timestamp and windowSize, fetches surrounding ciphertext window via API, decrypts in RAM with auth tag verification, returns structured envelope.
...
- **Parameters:**
  ...
  - `startDate` (string, optional): ISO-8601 UTC date string.
  - `endDate` (string, optional): ISO-8601 UTC date string.
- **Fail-Fast Invariant:** Throws `PROJECT_LOCKED` or `SESSION_EXPIRED` if not unlocked.
- **Implementation (Client-Side Zero-Knowledge):**
  1. Passes search boundaries (`startDate`, `endDate`, `limit`) to `POST /api/ai-chat/query` using cached token and idempotency key.
  2. Decrypts messages entirely in local client RAM with auth tag validation.
