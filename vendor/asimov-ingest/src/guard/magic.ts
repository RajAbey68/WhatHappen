 ...
 */
export function detectType(base64: string): DetectedType | null {
  ...
  if (ftyp === 'ftypheic' || ftyp === 'ftypheix' || ftyp === 'ftypmif1') return 'heic'
...
export function assertImageMagicBytes(base64: string): void {
  ...
  if (type === 'jpeg' || type === 'png' || type === 'heic' || type === 'webp') return
 ...
 */
export function assertAllowedType(base64: string, accept: readonly DetectedType[]): void {
  if (accept.length === 0) {
    throw new UploadGuardError(
      'UNSUPPORTED_TYPE',
      'No file types are permitted by this pipeline (empty accept-list).'
    )
  }

  const type = detectType(base64)
  if (type === null) {
    throw new UploadGuardError('UNSUPPORTED_TYPE', 'File type could not be identified.')
  }
  if (!accept.includes(type)) {
    throw new UploadGuardError(
      'UNSUPPORTED_TYPE',
      `Unsupported file type "${type}". Permitted: ${accept.join(', ')}.`
    )
  }
}
