...
# ─────────────────────────────────────────────────────────────
# Runs both truth-test harnesses (Upload+ETL + Reporting) in
# two modes:
#
#   Direct mode (default):
#     MOCK_APIS=true tsx tests/truth-tests/upload-etl-harness.ts
#     MOCK_APIS=true tsx tests/truth-tests/reporting-harness.ts
#     Imports route handlers directly, no server needed.
#
#   Server mode:
#     Starts Next.js dev server in background, runs harnesses
...
HARNESSES=(
  "$PROJECT_DIR/tests/truth-tests/upload-etl-harness.ts"
  "$PROJECT_DIR/tests/truth-tests/reporting-harness.ts"
)
NEXT_DEV_PID=""
...
for HARNESS in "${HARNESSES[@]}"; do
  HARNESS_NAME=$(basename "$HARNESS" .ts)
  echo -e "${CYAN}═══════════════════════════════════════════${NC}"
  echo -e "${CYAN}  Running: ${HARNESS_NAME}${NC}"
  echo -e "${CYAN}═══════════════════════════════════════════${NC}"
  echo ""
  ...
  if [ "$MODE" = "server" ]; then
    ...
    # Cleanup (only after first harness in server mode)
    echo ""
  ...
  # Extract test counts from output (rough heuristic)
  echo ""
  echo -e "${CYAN}───────────────────────────────────────────${NC}"
...
echo ""
echo -e "${CYAN}════════════════════════════════════════════${NC}"
echo -e "${CYAN}  ALL TRUTH TEST HARNESSES COMPLETE${NC}"
echo -e "${CYAN}════════════════════════════════════════════${NC}"

if [ $OVERALL_EXIT -eq 0 ]; then
  echo -e "${GREEN}  ALL TRUTH TESTS PASSED${NC}"
else
  echo -e "${RED}  SOME TRUTH TESTS FAILED (exit code $OVERALL_EXIT)${NC}"
fi
echo ""
